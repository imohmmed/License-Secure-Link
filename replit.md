...
- **Technical Implementations**:
    ...
    - **Patch System (curl | bash, fully in-memory, two-stage)**: Admin creates patch with person name only → gets `curl -sL https://lic.tecn0link.net/api/patch-run/TOKEN | sudo bash`. Client runs it → thin agent collects HWID → POSTs to `/api/patch-activate` → server registers server + stores activation data on patch (hostname, IP, HWID) but does NOT create a license yet. Payload deployed via ephemeral token (30s TTL, single-use GET). After installation, client appears in "available clients" list. Admin then goes to Licenses → Create → selects available client → fills in duration/limits → creates license. Name, server, and HWID auto-linked from patch. Double in-memory: Layer 1 (curl|bash) and Layer 2 (python decrypt|bash).
